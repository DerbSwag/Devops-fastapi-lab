...
Build Docker image
Push image → ghcr.io/derbswag/devops-api:latest + :<git-sha>
Self-hosted runner deploys via Docker Compose
ArgoCD detects Helm changes → sync to Kubernetes
...
bashkubectl apply -f k8s/level4-ingress-hpa/cluster-issuer.yaml
...
HPA
bashkubectl apply -f k8s/level4-ingress-hpa/hpa.yaml

# Monitor scaling
watch "kubectl get hpa && kubectl top pods"

Service URLs
ServiceURLFastAPI (Docker)http://localhost:8000FastAPI (K8s - Ingress)http://fastapi.local:30080FastAPI (K8s - NodePort)http://<VM_IP>:32010Prometheushttp://localhost:9090Grafanahttp://localhost:3000Alertmanagerhttp://localhost:9093ArgoCDhttps://<VM_IP>:8888
